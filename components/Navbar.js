...
const Navbar = ({ addToCart, removeFromCart, clearCart, subTotal, cart, user }) => {
  ...
  return (
    <div className='bg-slate-100'>
      ...
      <div className="flex justify-between px-10 w-screen relative gap-4 md:gap-4 py-3 md:py-2 bg-slate-100 shadow-lg mb-3  md:justify-between md:flex-row ">
        <div className='flex gap-3  flex-col md:flex-row  justify-center items-center'>
          <div className='flex justify-between w-full'>
            
            <div className="logo w-fit">
              <Link href={'/'} className='w-fit'>
                <Image src={'/title.png'} width={200} height={50} alt='' className='h-10 w-20 md:h-14 md:w-24' />
              </Link>
            </div>
            <div className="cart">
              <div className=' px-6 py-2 font-bold text-3xl rounded flex gap-6 justify-center items-center absolute right-0 top-[5%] md:top-[20%] md:right-[1%] flex-col md:flex-row'>
                <span>
                  {dropdown && <div onMouseLeave={() => setDropdown(false)} className="text-sm  font-semibold absolute right-12 top-12 bg-white shadow-md border  rounded-md py-2 px-3 w-36 md:w-54" >
                    <ul className='flex flex-col gap-4'>
                      <Link href={'/OrderList'}><li className='hover:underline hover:text-pink-500 cursor-pointer'>Orders</li></Link>
                  ...
                  {user && user.value ? <div > <MdAccountCircle onMouseEnter={handleDropdown} className='hover:text-[#FF87A5] text-base md:text-2xl' /></div> : ""}
                </span>
                ...
                <FaCartShopping onClick={toggleCart} className='hover:text-[#FF87A5] text-base md:text-lg' />
              </div>
          ...
          <div className="nav">
            <ul className='flex gap-4   font-semibold'>
              <Link href={'/tshirts'}><li className='text-base md:text-xl'>Tshirts</li></Link>
              <Link href={'/hoodies'}><li className='text-base md:text-xl'>Hoodies</li></Link>
              <Link href={'/stickers'}><li className='text-base md:text-xl'>Stickers</li></Link>
              <Link href={'/mugs'}><li className='text-base md:text-xl'>Mugs</li></Link>
            </ul>
          </div>
        </div>
        {/* <div className="cart">
          <div className=' px-6 py-2 font-bold text-3xl rounded  relative flex gap-6  md:flex-row'>
            <a>
              {dropdown && <div onMouseLeave={() => setDropdown(false)} className="text-sm  font-semibold absolute right-12 top-12 bg-white shadow-md border  rounded-md py-2 px-3 w-36 md:w-54" >
                <ul className='flex flex-col gap-4'>
